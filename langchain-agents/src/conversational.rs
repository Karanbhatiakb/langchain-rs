//! Conversational agent implementation with chat history support.
...
impl ConversationalAgent {
    ...
    fn default_prompt(tools: &[Arc<dyn BaseTool>]) -> PromptTemplate {
        ...
        let template = format!(
            "You are a conversational AI assistant with access to the following tools:\n\n\
            {tool_descriptions}\n\n\
    ...
    fn parse_output(text: &str) -> Result<AgentNextStep> {
        ...
        Err(ChainError::ParserError(format!(
            "Could not parse ConversationalAgent output: {}",
            text
        )))
    }
...
impl Agent for ConversationalAgent {
    ...
    async fn plan(
        ...
        let messages = vec![
            SystemMessage::new("You are a conversational AI assistant. Use tools when needed, and consider the conversation history.").into(),
    ...
    async fn return_stopped_response(
        ...
        match early_stopping_method {
            "force" => {
                ...
                return_values.insert(
                    ...
                    Value::String(
                        intermediate_steps
                            .last()
                            .map_or_else(|| "Agent stopped due to max iterations".to_string(), |s| {
                                s.observation.clone()
                            }),
                    ),
                );
                Ok(AgentFinish {
                    return_values,
                    log: "Agent stopped due to force early stopping".to_string(),
                })
            }
            "generate" => {
                let scratchpad = self.construct_scratchpad(intermediate_steps);
                ...
                let prompt = self.prompt.format(&kwargs)?;
                let messages = vec![
                    SystemMessage::new("Produce a final answer given the observations and conversation history.").into(),
                    HumanMessage::new(&prompt).into(),
                ];
                let response = self.llm.predict_messages(&messages, None, None).await?;
            ...
            _ => {
                ...
                Ok(AgentFinish {
                    return_values,
                    log: format!("Agent stopped via method: {}", early_stopping_method),
                })
            }
        }
    }
