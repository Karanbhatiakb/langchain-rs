//! ReAct agent implementation (reason + act).
        ...
        match early_stopping_method {
            "force" => {
                ...
                return_values.insert(
                    "output".to_string(),
                    Value::String(intermediate_steps.last().map_or_else(
                        || "Agent stopped due to max iterations".to_string(),
                        |s| s.observation.clone(),
                    )),
                );
                Ok(AgentFinish {
                    return_values,
            ...
            "generate" => {
                ...
                let prompt = self.prompt.format(&kwargs)?;
                let messages = vec![
                    SystemMessage::new("Produce a final answer given the observations.").into(),
