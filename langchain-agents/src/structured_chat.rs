...
impl StructuredChatAgent {
    ...
    fn default_prompt(tools: &[Arc<dyn BaseTool>]) -> PromptTemplate {
        ...
        let template = format!(
            ...
            {tool_descriptions}\n\n\
            You must respond with JSON in one of the following formats:\n\n\
            To use a tool:\n\
    ...
    fn parse_output(text: &str) -> Result<AgentNextStep> {
        ...
        let json_start = trimmed
            .find('{')
            .ok_or_else(|| ChainError::ParserError("No JSON found in output".to_string()))?;
        let json_str = &trimmed[json_start..];

        let json_end = json_str
            .rfind('}')
            .ok_or_else(|| ChainError::ParserError("Unclosed JSON in output".to_string()))?;
        ...
        Err(ChainError::ParserError(format!(
            "Could not parse structured chat JSON: {}",
            text
        )))
    }
}
