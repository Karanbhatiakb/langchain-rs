...
impl XMLAgent {
    ...
    fn default_prompt(tools: &[Arc<dyn BaseTool>]) -> PromptTemplate {
        let tool_descriptions: String = tools
            .iter()
            .map(|t| format!("<tool_name>{}</tool_name> - {}", t.name(), t.description()))
            .collect::<Vec<_>>()
            .join("\n");
        ...
        let template = format!(
            ...
            <tool_name>the tool name</tool_name>\n\
            <input>the input to the tool</input>\n\n\
            To provide a final answer, use:\n\n\
    ...
    fn parse_output(text: &str) -> Result<AgentNextStep> {
        let final_re =
            Regex::new(r"<final_answer>(.*?)</final_answer>")
        ...
        let action_re =
            Regex::new(r"<tool_name>(.*?)</tool_name>\s*<input>(.*?)</input>")
                .map_err(|e| ChainError::ParserError(e.to_string()))?;
        ...
        Err(ChainError::ParserError(format!(
            "Could not parse XML agent output: {}",
            text
        )))
    }
}
...
impl Agent for XMLAgent {
    ...
    async fn plan(
        ...
        let messages = vec![
            SystemMessage::new("You are a helpful AI assistant. Respond in XML format.").into(),
