...
//!
//! Ark embedding models for text vectorization.
...
/// # Example
/// ```ignore
/// use langchain_embeddings::providers::ark::ArkEmbeddings;
/// use langchain_core::traits::Embeddings;
///
/// let emb = ArkEmbeddings::new();
/// let vec = emb.embed_query("hello").await.unwrap();
/// ```
#[derive(Debug, Clone)]
pub struct ArkEmbeddings {
    _private: (),
}

impl ArkEmbeddings {
    /// Creates a new `ArkEmbeddings` instance.
    pub fn new() -> Self {
        Self { _private: () }
    }
}

impl Default for ArkEmbeddings {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Embeddings for ArkEmbeddings {
