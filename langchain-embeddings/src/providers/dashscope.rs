...
//!
//! DashScope (Alibaba Cloud) embedding models via their API. This is
...
/// # Example
/// ```ignore
/// use langchain_embeddings::providers::dashscope::DashScopeEmbeddings;
/// use langchain_core::traits::Embeddings;
///
/// let emb = DashScopeEmbeddings::new();
/// let vec = emb.embed_query("hello").await.unwrap();
/// ```
#[derive(Debug, Clone)]
pub struct DashScopeEmbeddings {
    _private: (),
}

impl DashScopeEmbeddings {
    /// Creates a new `DashScopeEmbeddings` instance.
    pub fn new() -> Self {
        Self { _private: () }
    }
}

impl Default for DashScopeEmbeddings {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Embeddings for DashScopeEmbeddings {
