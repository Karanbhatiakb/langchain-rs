//! AI21 Labs (Jurassic) LLM provider implementation.
...
const AI21_BASE_URL: &str = "https://api.ai21.com/studio/v1";
...
impl std::fmt::Debug for Ai21LLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Ai21LLM")
...
impl std::fmt::Debug for ChatAi21 {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ChatAi21")
            .field("model", &self.model)
            .field("base_url", &self.base_url)
            .finish()
    }
...
impl Ai21LLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        Self {
            ...
            base_url: AI21_BASE_URL.to_string(),
    ...
    fn convert_messages(&self, prompts: &[String]) -> Vec<Ai21Message> {
        prompts
            .iter()
            .map(|p| Ai21Message {
                role: "user".to_string(),
    ...
    fn build_request(&self, messages: Vec<Ai21Message>, stop: Option<&[&str]>, stream: bool) -> Ai21ChatRequest {
        Ai21ChatRequest {
    ...
    async fn send_request(&self, request: Ai21ChatRequest) -> Result<Ai21ChatResponse> {
            ...
            match response {
                Ok(resp) => {
                    if resp.status().is_success() {
                        return Ok(resp.json::<Ai21ChatResponse>().await.map_err(|e| {
                    ...
                    if status == reqwest::StatusCode::TOO_MANY_REQUESTS {
                        last_error = Some(ChainError::RateLimitError(
                            "Rate limited by AI21 API".to_string(),
                        ));
            ...
            while let Some(chunk) = stream.next().await {
                ...
                while let Some(line_end) = buffer.find('\n') {
                    ...
                    if let Some(data) = line.strip_prefix("data: ") {
                        if let Ok(chunk_data) = serde_json::from_str::<Ai21StreamChunk>(data) {
...
impl ChatAi21 {
    ...
    fn convert_messages(&self, messages: &[BaseMessage]) -> Vec<Ai21Message> {
        messages
            .iter()
            .map(|msg| {
