...
const BASE_URL: &str = "https://api.ai21.com/v2";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Ai21V2LLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl Ai21V2LLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for AI21 v2");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for Ai21V2LLM {
    fn provider_name(&self) -> &'static str {
        "ai21_v2"
    }
}

impl ProviderConfig for Ai21V2LLM {
...
#[async_trait]
impl BaseLLM for Ai21V2LLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("AI21 v2 stub response for model: {}", self.model);
        Ok(LLMResult {
