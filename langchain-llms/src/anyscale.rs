...
const ANYSCALE_BASE_URL: &str = "https://api.endpoints.anyscale.com/v1";

pub struct AnyscaleLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for AnyscaleLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AnyscaleLLM")
...
impl AnyscaleLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Anyscale Endpoints");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: ANYSCALE_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for AnyscaleLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Anyscale stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for AnyscaleLLM {
