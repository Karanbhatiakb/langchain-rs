...
const BASE_URL: &str = "https://api.endpoints.anyscale.com/v2";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AnyscaleV2LLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl AnyscaleV2LLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Anyscale v2");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for AnyscaleV2LLM {
    fn provider_name(&self) -> &'static str {
        "anyscale_v2"
    }
}

impl ProviderConfig for AnyscaleV2LLM {
...
#[async_trait]
impl BaseLLM for AnyscaleV2LLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Anyscale v2 stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for AnyscaleV2LLM {
