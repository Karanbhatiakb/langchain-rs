...
const BAICHUAN_BASE_URL: &str = "https://api.baichuan-ai.com/v1";

pub struct BaichuanLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for BaichuanLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BaichuanLLM")
...
impl BaichuanLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Baichuan");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: BAICHUAN_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for BaichuanLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Baichuan stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for BaichuanLLM {
