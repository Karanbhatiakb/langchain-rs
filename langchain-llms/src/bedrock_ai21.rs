...
const BEDROCK_AI21_BASE_URL: &str = "https://bedrock-runtime.us-east-1.amazonaws.com";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BedrockAi21LLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl BedrockAi21LLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Bedrock AI21");
        }
        Self {
            api_key,
            model: model.into(),
            temperature: 0.7,
            max_tokens: 1024,
            base_url: BEDROCK_AI21_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for BedrockAi21LLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Bedrock AI21 stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for BedrockAi21LLM {
