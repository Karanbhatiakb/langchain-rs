//! AWS Bedrock Amazon Titan LLM provider.
...
impl BedrockAmazonLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for Bedrock Amazon");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for BedrockAmazonLLM {
    fn provider_name(&self) -> &'static str {
        "bedrock_amazon"
    }
}

impl ProviderConfig for BedrockAmazonLLM {
...
#[async_trait]
impl BaseLLM for BedrockAmazonLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Bedrock Amazon stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for BedrockAmazonLLM {
