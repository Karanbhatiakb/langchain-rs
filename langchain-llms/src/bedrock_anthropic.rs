...
pub struct BedrockAnthropicLLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl BedrockAnthropicLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for Bedrock Anthropic");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for BedrockAnthropicLLM {
    fn provider_name(&self) -> &'static str {
        "bedrock_anthropic"
    }
}

impl ProviderConfig for BedrockAnthropicLLM {
...
#[async_trait]
impl BaseLLM for BedrockAnthropicLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Bedrock Anthropic stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for BedrockAnthropicLLM {
