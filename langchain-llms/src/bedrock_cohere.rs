...
const BEDROCK_COHERE_BASE_URL: &str = "https://bedrock-runtime.us-east-1.amazonaws.com";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BedrockCohereLLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl BedrockCohereLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for Bedrock Cohere");
        }
        Self {
            api_key,
            model: model.into(),
            temperature: 0.7,
            max_tokens: 1024,
            base_url: BEDROCK_COHERE_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for BedrockCohereLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Bedrock Cohere stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for BedrockCohereLLM {
