//! AWS Bedrock Meta Llama LLM provider.
...
impl BedrockMetaLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Bedrock Meta");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for BedrockMetaLLM {
    fn provider_name(&self) -> &'static str {
        "bedrock_meta"
    }
}

impl ProviderConfig for BedrockMetaLLM {
...
#[async_trait]
impl BaseLLM for BedrockMetaLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Bedrock Meta stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for BedrockMetaLLM {
