//! AWS Bedrock Stability AI LLM provider.
...
pub struct BedrockStabilityAiLLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl BedrockStabilityAiLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for Bedrock Stability AI");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for BedrockStabilityAiLLM {
    fn provider_name(&self) -> &'static str {
        "bedrock_stabilityai"
    }
}

impl ProviderConfig for BedrockStabilityAiLLM {
...
#[async_trait]
impl BaseLLM for BedrockStabilityAiLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Bedrock Stability AI stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for BedrockStabilityAiLLM {
