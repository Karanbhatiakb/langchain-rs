...
impl std::fmt::Debug for CerebrasLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CerebrasLLM")
...
impl std::fmt::Debug for ChatCerebras {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ChatCerebras")
            .field("model", &self.model)
            .field("base_url", &self.base_url)
            .finish()
    }
...
impl CerebrasLLM {
    ...
    fn convert_messages(&self, prompts: &[String]) -> Vec<CerebrasMessage> {
        prompts
            .iter()
            .map(|p| CerebrasMessage {
                role: "user".to_string(),
                content: p.clone(),
                name: None,
            })
    ...
    fn build_request(&self, messages: Vec<CerebrasMessage>, stop: Option<&[&str]>, stream: bool) -> CerebrasChatRequest {
        CerebrasChatRequest {
    ...
    async fn send_request(&self, request: CerebrasChatRequest) -> Result<CerebrasChatResponse> {
            ...
            match response {
                Ok(resp) => {
                    if resp.status().is_success() {
                        return Ok(resp.json::<CerebrasChatResponse>().await.map_err(|e| {
                    ...
                    if status == reqwest::StatusCode::TOO_MANY_REQUESTS {
                        last_error = Some(ChainError::RateLimitError(
                            "Rate limited by Cerebras API".to_string(),
                        ));
            ...
            while let Some(chunk) = stream.next().await {
                ...
                while let Some(line_end) = buffer.find('\n') {
                    ...
                    if let Some(data) = line.strip_prefix("data: ") {
                        if let Ok(chunk_data) = serde_json::from_str::<CerebrasStreamChunk>(data) {
...
impl ChatCerebras {
    ...
    fn convert_messages(&self, messages: &[BaseMessage]) -> Vec<CerebrasMessage> {
        messages
            .iter()
            .map(|msg| {
...
#[async_trait]
impl ChatModel for ChatCerebras {
