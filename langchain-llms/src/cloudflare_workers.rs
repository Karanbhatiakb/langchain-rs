...
const CLOUDFLARE_WORKERS_BASE_URL: &str = "https://api.cloudflare.com/client/v4/accounts";

pub struct CloudflareWorkersLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for CloudflareWorkersLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CloudflareWorkersLLM")
...
impl CloudflareWorkersLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for Cloudflare Workers AI");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: CLOUDFLARE_WORKERS_BASE_URL.to_string(),
...
impl Clone for CloudflareWorkersLLM {
    fn clone(&self) -> Self {
        Self {
...
#[async_trait]
impl BaseLLM for CloudflareWorkersLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Cloudflare Workers AI stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for CloudflareWorkersLLM {
