...
const COZE_BASE_URL: &str = "https://api.coze.com";
...
impl std::fmt::Debug for CozeLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CozeLLM")
...
impl CozeLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Coze");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: COZE_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for CozeLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Coze stub response for model: {}", self.model);
        Ok(LLMResult {
