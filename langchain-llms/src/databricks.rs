...
const DATABRICKS_BASE_URL: &str = "https://api.databricks.com";

pub struct DatabricksLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for DatabricksLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DatabricksLLM")
...
impl std::fmt::Debug for ChatDatabricks {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ChatDatabricks")
            .field("model", &self.model)
            .field("base_url", &self.base_url)
            .finish()
    }
...
struct DatabricksChatResponse {
    ...
    #[serde(default)]
    usage: Option<DatabricksUsage>,
}

#[derive(Deserialize)]
struct DatabricksChoice {
    index: u32,
    message: DatabricksResponseMessage,
    finish_reason: Option<String>,
}

#[derive(Deserialize)]
struct DatabricksResponseMessage {
    role: String,
...
struct DatabricksStreamChunk {
    choices: Vec<DatabricksStreamChoice>,
}

#[derive(Deserialize)]
struct DatabricksStreamChoice {
    delta: DatabricksDelta,
    finish_reason: Option<String>,
}

#[derive(Deserialize)]
struct DatabricksDelta {
    content: Option<String>,
    role: Option<String>,
}
...
impl DatabricksLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        Self {
            ...
            base_url: DATABRICKS_BASE_URL.to_string(),
    ...
    fn convert_messages(&self, prompts: &[String]) -> Vec<DatabricksMessage> {
        prompts
            .iter()
            .map(|p| DatabricksMessage {
                role: "user".to_string(),
                content: p.clone(),
                name: None,
            })
    ...
    fn build_request(&self, messages: Vec<DatabricksMessage>, stop: Option<&[&str]>, stream: bool) -> DatabricksChatRequest {
        DatabricksChatRequest {
    ...
    async fn send_request(&self, request: DatabricksChatRequest) -> Result<DatabricksChatResponse> {
            ...
            match response {
                Ok(resp) => {
                    if resp.status().is_success() {
                        return Ok(resp.json::<DatabricksChatResponse>().await.map_err(|e| {
                    ...
                    if status == reqwest::StatusCode::TOO_MANY_REQUESTS {
                        last_error = Some(ChainError::RateLimitError(
                            "Rate limited by Databricks API".to_string(),
                        ));
        ...
        let stream = async_stream::try_stream! {
            ...
            if !response.status().is_success() {
                ...
                let err = ChainError::LLMError(format!(
                    "Databricks API streaming error ({})",
                    status
                ));
                Err(err)?;
            }
            ...
            while let Some(chunk) = stream.next().await {
                ...
                while let Some(line_end) = buffer.find('\n') {
                    ...
                    if let Some(data) = line.strip_prefix("data: ") {
                        if let Ok(chunk_data) = serde_json::from_str::<DatabricksStreamChunk>(data) {
...
#[async_trait]
impl BaseLLM for DatabricksLLM {
...
impl ChatDatabricks {
    ...
    fn convert_messages(&self, messages: &[BaseMessage]) -> Vec<DatabricksMessage> {
        messages
            .iter()
            .map(|msg| {
...
#[async_trait]
impl BaseLLM for ChatDatabricks {
    async fn generate(&self, prompts: &[String], stop: Option<&[&str]>) -> Result<LLMResult> {
...
#[async_trait]
impl ChatModel for ChatDatabricks {
