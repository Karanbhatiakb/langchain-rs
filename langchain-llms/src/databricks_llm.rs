...
const BASE_URL: &str = "https://YOUR_WORKSPACE.cloud.databricks.com";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DatabricksLlmLLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl DatabricksLlmLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for Databricks LLM");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for DatabricksLlmLLM {
    fn provider_name(&self) -> &'static str {
        "databricks_llm"
    }
}

impl ProviderConfig for DatabricksLlmLLM {
...
#[async_trait]
impl BaseLLM for DatabricksLlmLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Databricks LLM stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for DatabricksLlmLLM {
