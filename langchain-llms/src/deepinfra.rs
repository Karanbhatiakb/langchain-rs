...
impl std::fmt::Debug for DeepInfraLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DeepInfraLLM")
...
impl DeepInfraLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for DeepInfra");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: DEEPINFRA_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for DeepInfraLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("DeepInfra stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for DeepInfraLLM {
