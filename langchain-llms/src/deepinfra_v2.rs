...
const BASE_URL: &str = "https://api.deepinfra.com/v2";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DeepInfraV2LLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl DeepInfraV2LLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for DeepInfra v2");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for DeepInfraV2LLM {
    fn provider_name(&self) -> &'static str {
        "deepinfra_v2"
    }
}

impl ProviderConfig for DeepInfraV2LLM {
...
#[async_trait]
impl BaseLLM for DeepInfraV2LLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("DeepInfra v2 stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for DeepInfraV2LLM {
