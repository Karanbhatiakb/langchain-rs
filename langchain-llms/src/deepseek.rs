...
impl std::fmt::Debug for ChatDeepSeek {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ChatDeepSeek")
...
impl ChatDeepSeek {
    ...
    fn convert_messages(&self, messages: &[BaseMessage]) -> Vec<DeepSeekMessage> {
        messages
            .iter()
            .map(|msg| {
    ...
    async fn send_request(&self, request: DeepSeekChatRequest) -> Result<DeepSeekChatResponse> {
        let response = self
            .client
        ...
        Ok(response.json::<DeepSeekChatResponse>().await?)
    }
    ...
    async fn stream_request(
        ...
        let stream = async_stream::try_stream! {
            let response = client
                ...
                .send()
                .await
                .map_err(|e| ChainError::LLMError(format!("DeepSeek stream request failed: {}", e)))?;

            if !response.status().is_success() {
                let status = response.status();
                let body = response.text().await.unwrap_or_default();
                Err(ChainError::LLMError(format!(
                    "DeepSeek API streaming error ({}): {}",
                    status, body
                )))?;
            }
