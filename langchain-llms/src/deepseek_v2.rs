...
const DEEPSEEK_V2_BASE_URL: &str = "https://api.deepseek.com/v2";

pub struct DeepSeekV2LLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for DeepSeekV2LLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DeepSeekV2LLM")
...
impl DeepSeekV2LLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for DeepSeek V2");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: DEEPSEEK_V2_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for DeepSeekV2LLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("DeepSeek V2 stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for DeepSeekV2LLM {
