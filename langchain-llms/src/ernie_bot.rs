...
const ERNIE_BOT_BASE_URL: &str = "https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/chat";

pub struct ErnieBotLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for ErnieBotLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ErnieBotLLM")
...
impl ErnieBotLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Baidu ERNIE Bot");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: ERNIE_BOT_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for ErnieBotLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("ErnieBot stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for ErnieBotLLM {
