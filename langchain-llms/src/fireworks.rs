...
impl std::fmt::Debug for ChatFireworks {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ChatFireworks")
...
impl ChatFireworks {
    ...
    fn convert_messages(&self, messages: &[BaseMessage]) -> Vec<FireworksMessage> {
        messages
            .iter()
            .map(|msg| {
    ...
    async fn send_request(&self, request: FireworksChatRequest) -> Result<FireworksChatResponse> {
        let response = self
            .client
            ...
            .send()
            .await
            .map_err(|e| ChainError::LLMError(format!("Fireworks request failed: {}", e)))?;
        ...
        Ok(response.json::<FireworksChatResponse>().await?)
    }
    ...
    async fn stream_request(
        ...
        let stream = async_stream::try_stream! {
            let response = client
                ...
                .send()
                .await
                .map_err(|e| ChainError::LLMError(format!("Fireworks stream request failed: {}", e)))?;

            if !response.status().is_success() {
                let status = response.status();
                let body = response.text().await.unwrap_or_default();
                Err(ChainError::LLMError(format!(
                    "Fireworks API streaming error ({}): {}",
                    status, body
                )))?;
            }
            ...
            while let Some(chunk) = stream.next().await {
                ...
                while let Some(line_end) = buffer.find('\n') {
                    ...
                    if let Some(data) = line.strip_prefix("data: ") {
                        if let Ok(chunk_data) = serde_json::from_str::<FireworksStreamChunk>(data) {
...
#[async_trait]
impl BaseLLM for ChatFireworks {
    ...
    async fn stream(
        ...
        let messages: Vec<BaseMessage> = prompts
            ...
            .map(|p| BaseMessage::new(p.clone(), MessageType::Human))
            .collect();
        let fw_messages = self.convert_messages(&messages);
