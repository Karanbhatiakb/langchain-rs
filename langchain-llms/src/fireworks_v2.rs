...
const BASE_URL: &str = "https://api.fireworks.ai/inference/v2";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FireworksV2LLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl FireworksV2LLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Fireworks AI v2");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for FireworksV2LLM {
    fn provider_name(&self) -> &'static str {
        "fireworks_v2"
    }
}

impl ProviderConfig for FireworksV2LLM {
...
#[async_trait]
impl BaseLLM for FireworksV2LLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Fireworks AI v2 stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for FireworksV2LLM {
