...
const GOOGLE_PALM_BASE_URL: &str = "https://generativelanguage.googleapis.com/v1beta2";

pub struct GooglePalmLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for GooglePalmLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("GooglePalmLLM")
...
impl GooglePalmLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Google PaLM");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: GOOGLE_PALM_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for GooglePalmLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Google PaLM stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for GooglePalmLLM {
