...
const BASE_URL: &str = "https://generativelanguage.googleapis.com/v2";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GoogleV2LLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl GoogleV2LLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Google AI v2");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for GoogleV2LLM {
    fn provider_name(&self) -> &'static str {
        "google_v2"
    }
}

impl ProviderConfig for GoogleV2LLM {
...
#[async_trait]
impl BaseLLM for GoogleV2LLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Google AI v2 stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for GoogleV2LLM {
