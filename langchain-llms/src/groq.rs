...
impl std::fmt::Debug for ChatGroq {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ChatGroq")
...
impl ChatGroq {
    ...
    fn convert_messages(&self, messages: &[BaseMessage]) -> Vec<GroqMessage> {
        messages
            .iter()
            .map(|msg| {
    ...
    async fn send_request(&self, request: GroqChatRequest) -> Result<GroqChatResponse> {
        let response = self
            .client
            ...
            .send()
            .await
            .map_err(|e| ChainError::LLMError(format!("Groq request failed: {}", e)))?;
        ...
        Ok(response.json::<GroqChatResponse>().await?)
    }
    ...
    async fn stream_request(
        ...
        let stream = async_stream::try_stream! {
            let response = client
                ...
                .send()
                .await
                .map_err(|e| ChainError::LLMError(format!("Groq stream request failed: {}", e)))?;

            if !response.status().is_success() {
                let status = response.status();
                let body = response.text().await.unwrap_or_default();
                Err(ChainError::LLMError(format!(
                    "Groq API streaming error ({}): {}",
                    status, body
                )))?;
            }
            ...
            while let Some(chunk) = stream.next().await {
                ...
                while let Some(line_end) = buffer.find('\n') {
                    ...
                    if let Some(data) = line.strip_prefix("data: ") {
                        if let Ok(chunk_data) = serde_json::from_str::<GroqStreamChunk>(data) {
