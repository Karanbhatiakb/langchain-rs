...
const HUGGINGFACE_HUB_BASE_URL: &str = "https://api-inference.huggingface.co/v1";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HuggingFaceHubLLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl HuggingFaceHubLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for HuggingFace Hub");
        }
        Self {
            api_key,
            model: model.into(),
            temperature: 0.7,
            max_tokens: 1024,
            base_url: HUGGINGFACE_HUB_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for HuggingFaceHubLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("HuggingFace Hub stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for HuggingFaceHubLLM {
