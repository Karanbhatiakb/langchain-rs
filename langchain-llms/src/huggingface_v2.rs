...
const BASE_URL: &str = "https://api-inference.huggingface.co/v2";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HuggingFaceV2LLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl HuggingFaceV2LLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for HuggingFace v2");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for HuggingFaceV2LLM {
    fn provider_name(&self) -> &'static str {
        "huggingface_v2"
    }
}

impl ProviderConfig for HuggingFaceV2LLM {
...
#[async_trait]
impl BaseLLM for HuggingFaceV2LLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("HuggingFace v2 stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for HuggingFaceV2LLM {
