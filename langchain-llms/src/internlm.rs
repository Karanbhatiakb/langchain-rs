...
const INTERN_LM_BASE_URL: &str = "https://api.intern-ai.org.cn/v1";

pub struct InternLmLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for InternLmLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("InternLmLLM")
...
impl InternLmLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for InternLM");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: INTERN_LM_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for InternLmLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("InternLM stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for InternLmLLM {
