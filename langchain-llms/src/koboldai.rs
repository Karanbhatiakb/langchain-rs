...
const KOBOLDAI_BASE_URL: &str = "http://localhost:5000/api/v1";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct KoboldAiLLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl KoboldAiLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for KoboldAI");
        }
        Self {
            api_key,
            model: model.into(),
            temperature: 0.7,
            max_tokens: 1024,
            base_url: KOBOLDAI_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for KoboldAiLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("KoboldAI stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for KoboldAiLLM {
