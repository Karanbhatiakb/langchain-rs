//! LiteLLM provider (proxy/router for multiple backends).
...
impl std::fmt::Debug for LiteLLMLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LiteLLMLLM")
...
impl std::fmt::Debug for ChatLiteLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ChatLiteLLM")
            .field("model", &self.model)
            .field("base_url", &self.base_url)
            .finish()
    }
...
#[derive(Deserialize)]
struct LiteLLMStreamChunk {
    choices: Vec<LiteLLMStreamChoice>,
}

#[derive(Deserialize)]
struct LiteLLMStreamChoice {
    delta: LiteLLMDelta,
    finish_reason: Option<String>,
}
...
impl LiteLLMLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        Self {
            ...
            base_url: LITELLM_BASE_URL.to_string(),
    ...
    fn convert_messages(&self, prompts: &[String]) -> Vec<LiteLLMMessage> {
        prompts
            .iter()
            .map(|p| LiteLLMMessage {
                role: "user".to_string(),
                content: p.clone(),
                name: None,
            })
    ...
    fn build_request(&self, messages: Vec<LiteLLMMessage>, stop: Option<&[&str]>, stream: bool) -> LiteLLMChatRequest {
        LiteLLMChatRequest {
    ...
    async fn send_request(&self, request: LiteLLMChatRequest) -> Result<LiteLLMChatResponse> {
            ...
            match response {
                Ok(resp) => {
                    if resp.status().is_success() {
                        return Ok(resp.json::<LiteLLMChatResponse>().await.map_err(|e| {
                    ...
                    if status == reqwest::StatusCode::TOO_MANY_REQUESTS {
                        last_error = Some(ChainError::RateLimitError(
                            "Rate limited by LiteLLM proxy".to_string(),
                        ));
            ...
            while let Some(chunk) = stream.next().await {
                ...
                while let Some(line_end) = buffer.find('\n') {
                    ...
                    if let Some(data) = line.strip_prefix("data: ") {
                        if let Ok(chunk_data) = serde_json::from_str::<LiteLLMStreamChunk>(data) {
...
impl ChatLiteLLM {
    ...
    fn convert_messages(&self, messages: &[BaseMessage]) -> Vec<LiteLLMMessage> {
        messages
            .iter()
            .map(|msg| {
...
#[async_trait]
impl ChatModel for ChatLiteLLM {
