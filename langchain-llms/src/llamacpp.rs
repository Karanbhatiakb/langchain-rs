...
impl LlamaCppLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for LlamaCpp — running locally");
        }
        Self {
            model: model.into(),
            api_key,
...
#[async_trait]
impl BaseLLM for LlamaCppLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("LlamaCpp stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for LlamaCppLLM {
