...
const BASE_URL: &str = "https://YOUR_RESOURCE_NAME.openai.azure.com";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MicrosoftOpenAiLLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl MicrosoftOpenAiLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for Microsoft Azure OpenAI");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for MicrosoftOpenAiLLM {
    fn provider_name(&self) -> &'static str {
        "microsoft_openai"
    }
}

impl ProviderConfig for MicrosoftOpenAiLLM {
...
#[async_trait]
impl BaseLLM for MicrosoftOpenAiLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Microsoft Azure OpenAI stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for MicrosoftOpenAiLLM {
