...
impl std::fmt::Debug for MoonshotLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("MoonshotLLM")
...
impl MoonshotLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Moonshot AI");
        }
        Self {
            model: model.into(),
            api_key,
...
#[async_trait]
impl BaseLLM for MoonshotLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Moonshot AI stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for MoonshotLLM {
