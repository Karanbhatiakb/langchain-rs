...
const NLPLANET_BASE_URL: &str = "https://api.nlplanet.com/v1";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NLPlanetLLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl NLPlanetLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for NLPlanet");
        }
        Self {
            api_key,
            model: model.into(),
            temperature: 0.7,
            max_tokens: 1024,
            base_url: NLPLANET_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for NLPlanetLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("NLPlanet stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for NLPlanetLLM {
