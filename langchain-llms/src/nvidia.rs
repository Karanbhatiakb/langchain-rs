...
impl std::fmt::Debug for NvidiaLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("NvidiaLLM")
...
impl std::fmt::Debug for ChatNvidia {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ChatNvidia")
            .field("model", &self.model)
            .field("base_url", &self.base_url)
            .finish()
    }
...
struct NvidiaChoice {
    index: u32,
    message: NvidiaResponseMessage,
    finish_reason: Option<String>,
}

#[derive(Deserialize)]
struct NvidiaResponseMessage {
    role: String,
...
impl NvidiaLLM {
    ...
    fn convert_messages(&self, prompts: &[String]) -> Vec<NvidiaMessage> {
        prompts
            .iter()
            .map(|p| NvidiaMessage {
                role: "user".to_string(),
    ...
    fn build_request(&self, messages: Vec<NvidiaMessage>, stop: Option<&[&str]>, stream: bool) -> NvidiaChatRequest {
        NvidiaChatRequest {
    ...
    async fn send_request(&self, request: NvidiaChatRequest) -> Result<NvidiaChatResponse> {
            ...
            match response {
                Ok(resp) => {
                    if resp.status().is_success() {
                        return Ok(resp.json::<NvidiaChatResponse>().await.map_err(|e| {
            ...
            while let Some(chunk) = stream.next().await {
                ...
                while let Some(line_end) = buffer.find('\n') {
                    ...
                    if let Some(data) = line.strip_prefix("data: ") {
                        if let Ok(chunk_data) = serde_json::from_str::<NvidiaStreamChunk>(data) {
...
impl ChatNvidia {
    ...
    fn convert_messages(&self, messages: &[BaseMessage]) -> Vec<NvidiaMessage> {
        messages
            .iter()
            .map(|msg| {
