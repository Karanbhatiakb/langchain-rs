...
const OCTOAI_BASE_URL: &str = "https://text.octoai.run/v1";

pub struct OctoAiLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for OctoAiLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OctoAiLLM")
...
impl OctoAiLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for OctoAI");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: OCTOAI_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for OctoAiLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("OctoAI stub response for model: {}", self.model);
        Ok(LLMResult {
