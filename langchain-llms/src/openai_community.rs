...
const OPENAI_COMMUNITY_BASE_URL: &str = "https://api.openai.com/v1";

pub struct OpenAICommunityLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for OpenAICommunityLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OpenAICommunityLLM")
...
impl OpenAICommunityLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for OpenAI Community");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: OPENAI_COMMUNITY_BASE_URL.to_string(),
...
impl Clone for OpenAICommunityLLM {
    fn clone(&self) -> Self {
        Self {
...
#[async_trait]
impl BaseLLM for OpenAICommunityLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("OpenAI Community stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for OpenAICommunityLLM {
