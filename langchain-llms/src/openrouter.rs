...
impl std::fmt::Debug for OpenRouterLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OpenRouterLLM")
...
impl std::fmt::Debug for ChatOpenRouter {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ChatOpenRouter")
            .field("model", &self.model)
            .field("base_url", &self.base_url)
            .finish()
    }
...
fn build_headers(api_key: &str, site_url: Option<&str>, site_name: Option<&str>) -> HashMap<String, String> {
    let mut headers = HashMap::new();
    headers.insert("Authorization".to_string(), format!("Bearer {}", api_key));
    headers.insert("Content-Type".to_string(), "application/json".to_string());
    if let Some(url) = site_url {
        headers.insert("HTTP-Referer".to_string(), url.to_string());
    }
    if let Some(name) = site_name {
        headers.insert("X-Title".to_string(), name.to_string());
    }
    headers
}

macro_rules! impl_openrouter_shared {
    ($ty:ty, $is_llm:expr) => {
        impl $ty {
            fn build_request_headers(&self) -> HashMap<String, String> {
                build_headers(&self.api_key, self.site_url.as_deref(), self.site_name.as_deref())
            }
                    ...
                    match response {
                        Ok(resp) => {
                            if resp.status().is_success() {
                                return Ok(resp.json::<OpenRouterChatResponse>().await.map_err(|e| {
                            ...
                            if status == reqwest::StatusCode::TOO_MANY_REQUESTS {
                                last_error = Some(ChainError::RateLimitError(
                                    "Rate limited by OpenRouter API".to_string(),
                                ));
                    ...
                    while let Some(chunk) = stream.next().await {
                        ...
                        while let Some(line_end) = buffer.find('\n') {
                            ...
                            if let Some(data) = line.strip_prefix("data: ") {
                                if let Ok(chunk_data) = serde_json::from_str::<OpenRouterStreamChunk>(data) {
...
impl_openrouter_shared!(OpenRouterLLM, true);
impl_openrouter_shared!(ChatOpenRouter, false);
...
impl OpenRouterLLM {
    ...
    fn convert_messages(&self, prompts: &[String]) -> Vec<OpenRouterMessage> {
        prompts
            .iter()
            .map(|p| OpenRouterMessage {
                role: "user".to_string(),
                content: p.clone(),
                name: None,
            })
    ...
    fn build_request(&self, messages: Vec<OpenRouterMessage>, stop: Option<&[&str]>, stream: bool) -> OpenRouterChatRequest {
...
impl BaseLLM for OpenRouterLLM {
    async fn generate(&self, prompts: &[String], stop: Option<&[&str]>) -> Result<LLMResult> {
        ...
        let response = Self::send_request_inner(
            &self.base_url,
            &headers,
            &self.client,
            self.timeout,
...
impl ChatOpenRouter {
    ...
    fn convert_messages(&self, messages: &[BaseMessage]) -> Vec<OpenRouterMessage> {
        messages
            .iter()
            .map(|msg| {
...
#[async_trait]
impl BaseLLM for ChatOpenRouter {
    async fn generate(&self, prompts: &[String], stop: Option<&[&str]>) -> Result<LLMResult> {
...
#[async_trait]
impl ChatModel for ChatOpenRouter {
    ...
    async fn stream_messages(
        ...
        let request = self.build_request(converted, stop, true);
        let headers = self.build_request_headers();
        let stream = Self::stream_request_inner(
            &self.base_url,
            &headers,
            &self.client,
            self.timeout,
