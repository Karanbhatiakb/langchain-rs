...
const BASE_URL: &str = "https://openrouter.ai/api/v2";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct OpenRouterV2LLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl OpenRouterV2LLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for OpenRouter v2");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for OpenRouterV2LLM {
    fn provider_name(&self) -> &'static str {
        "openrouter_v2"
    }
}

impl ProviderConfig for OpenRouterV2LLM {
...
#[async_trait]
impl BaseLLM for OpenRouterV2LLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("OpenRouter v2 stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for OpenRouterV2LLM {
