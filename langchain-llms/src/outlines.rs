...
const OUTLINES_BASE_URL: &str = "http://localhost:8000";

pub struct OutlinesLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for OutlinesLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OutlinesLLM")
...
impl OutlinesLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Outlines");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: OUTLINES_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for OutlinesLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Outlines stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for OutlinesLLM {
