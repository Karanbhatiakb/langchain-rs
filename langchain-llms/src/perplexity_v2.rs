...
const BASE_URL: &str = "https://api.perplexity.ai/v2";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PerplexityV2LLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl PerplexityV2LLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for Perplexity v2");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for PerplexityV2LLM {
    fn provider_name(&self) -> &'static str {
        "perplexity_v2"
    }
}

impl ProviderConfig for PerplexityV2LLM {
...
#[async_trait]
impl BaseLLM for PerplexityV2LLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Perplexity v2 stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for PerplexityV2LLM {
