...
const PREDIBASE_BASE_URL: &str = "https://api.predibase.com/v1";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PredibaseLLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl PredibaseLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Predibase");
        }
        Self {
            api_key,
            model: model.into(),
            temperature: 0.7,
            max_tokens: 1024,
            base_url: PREDIBASE_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for PredibaseLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Predibase stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for PredibaseLLM {
