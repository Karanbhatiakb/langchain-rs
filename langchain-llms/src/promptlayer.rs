...
const PROMPTLAYER_BASE_URL: &str = "https://api.promptlayer.com";

pub struct PromptLayerLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for PromptLayerLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PromptLayerLLM")
...
impl PromptLayerLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for PromptLayer");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: PROMPTLAYER_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for PromptLayerLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("PromptLayer stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for PromptLayerLLM {
