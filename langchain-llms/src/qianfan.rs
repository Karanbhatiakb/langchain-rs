...
const QIANFAN_BASE_URL: &str = "https://aip.baidubce.com/rpc/2.0/ai_custom";

pub struct QianfanLLM {
    model: String,
    api_key: String,
    secret_key: String,
    base_url: String,
...
impl std::fmt::Debug for QianfanLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("QianfanLLM")
...
impl QianfanLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Baidu Qianfan");
        }
        Self {
            model: model.into(),
            api_key,
            secret_key: String::new(),
            base_url: QIANFAN_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for QianfanLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Qianfan stub response for model: {}", self.model);
        Ok(LLMResult {
