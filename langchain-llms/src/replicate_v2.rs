...
const BASE_URL: &str = "https://api.replicate.com/v2";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ReplicateV2LLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl ReplicateV2LLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Replicate v2");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for ReplicateV2LLM {
    fn provider_name(&self) -> &'static str {
        "replicate_v2"
    }
}

impl ProviderConfig for ReplicateV2LLM {
...
#[async_trait]
impl BaseLLM for ReplicateV2LLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Replicate v2 stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for ReplicateV2LLM {
