...
const SPARKDESK_BASE_URL: &str = "https://spark-api.xf-yun.com/v3.1/chat";

pub struct SparkDeskLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for SparkDeskLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SparkDeskLLM")
...
impl SparkDeskLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for iFlytek SparkDesk");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: SPARKDESK_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for SparkDeskLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("SparkDesk stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for SparkDeskLLM {
