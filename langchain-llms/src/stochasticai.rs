...
const STOCHASTIC_AI_BASE_URL: &str = "https://api.stochastic.ai/v1";

pub struct StochasticAiLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for StochasticAiLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("StochasticAiLLM")
...
impl StochasticAiLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for Stochastic AI");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: STOCHASTIC_AI_BASE_URL.to_string(),
...
impl Clone for StochasticAiLLM {
    fn clone(&self) -> Self {
        Self {
...
#[async_trait]
impl BaseLLM for StochasticAiLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Stochastic AI stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for StochasticAiLLM {
