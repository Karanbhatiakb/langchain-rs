...
const TEXT_GEN_WEBUI_BASE_URL: &str = "http://localhost:7860";

pub struct TextGenWebUiLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for TextGenWebUiLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TextGenWebUiLLM")
...
impl TextGenWebUiLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        let api_key = api_key.into();
        if api_key.is_empty() {
            warn!("No API key set for Text Generation WebUI");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: TEXT_GEN_WEBUI_BASE_URL.to_string(),
...
impl Clone for TextGenWebUiLLM {
    fn clone(&self) -> Self {
        Self {
...
#[async_trait]
impl BaseLLM for TextGenWebUiLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("TextGen WebUI stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for TextGenWebUiLLM {
