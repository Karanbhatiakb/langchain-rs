...
const BASE_URL: &str = "https://api.together.xyz/v2";

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TogetherV2LLM {
    api_key: String,
    model: String,
    temperature: f64,
    max_tokens: u32,
...
impl TogetherV2LLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Together AI v2");
        }
        Self {
            api_key,
            model: model.into(),
...
impl LLMProvider for TogetherV2LLM {
    fn provider_name(&self) -> &'static str {
        "together_v2"
    }
}

impl ProviderConfig for TogetherV2LLM {
...
#[async_trait]
impl BaseLLM for TogetherV2LLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Together AI v2 stub response for model: {}", self.model);
        Ok(LLMResult {
...
#[async_trait]
impl ChatModel for TogetherV2LLM {
