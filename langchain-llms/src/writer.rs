...
const WRITER_BASE_URL: &str = "https://api.writer.com/v1";
...
impl std::fmt::Debug for WriterLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("WriterLLM")
...
impl WriterLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Writer Palmyra");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: WRITER_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for WriterLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Writer Palmyra stub response for model: {}", self.model);
        Ok(LLMResult {
