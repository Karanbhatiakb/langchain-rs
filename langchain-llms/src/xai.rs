...
impl std::fmt::Debug for XaiLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("XaiLLM")
...
impl std::fmt::Debug for ChatXai {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ChatXai")
            .field("model", &self.model)
            .field("base_url", &self.base_url)
            .finish()
    }
...
impl XaiLLM {
    ...
    fn convert_messages(&self, prompts: &[String]) -> Vec<XaiMessage> {
        prompts
            .iter()
            .map(|p| XaiMessage {
                role: "user".to_string(),
    ...
    async fn send_request(&self, request: XaiChatRequest) -> Result<XaiChatResponse> {
            ...
            match response {
                Ok(resp) => {
                    if resp.status().is_success() {
                        return Ok(resp.json::<XaiChatResponse>().await.map_err(|e| {
                    ...
                    if status == reqwest::StatusCode::TOO_MANY_REQUESTS {
                        last_error = Some(ChainError::RateLimitError(
                            "Rate limited by xAI API".to_string(),
                        ));
            ...
            while let Some(chunk) = stream.next().await {
                ...
                while let Some(line_end) = buffer.find('\n') {
                    ...
                    if let Some(data) = line.strip_prefix("data: ") {
                        if let Ok(chunk_data) = serde_json::from_str::<XaiStreamChunk>(data) {
...
impl ChatXai {
    ...
    fn convert_messages(&self, messages: &[BaseMessage]) -> Vec<XaiMessage> {
        messages
            .iter()
            .map(|msg| {
