...
const YI_BASE_URL: &str = "https://api.01.ai/v1";
...
impl std::fmt::Debug for YiLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("YiLLM")
...
impl YiLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for Yi (01.AI)");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: YI_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for YiLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("Yi (01.AI) stub response for model: {}", self.model);
        Ok(LLMResult {
