...
const ZHIPU_AI_BASE_URL: &str = "https://open.bigmodel.cn/api/paas/v4";

pub struct ZhipuAiLLM {
    model: String,
    api_key: String,
    base_url: String,
...
impl std::fmt::Debug for ZhipuAiLLM {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ZhipuAiLLM")
...
impl ZhipuAiLLM {
    pub fn new(model: impl Into<String>, api_key: impl Into<String>) -> Self {
        ...
        if api_key.is_empty() {
            warn!("No API key set for ZhipuAI");
        }
        Self {
            model: model.into(),
            api_key,
            base_url: ZHIPU_AI_BASE_URL.to_string(),
...
#[async_trait]
impl BaseLLM for ZhipuAiLLM {
    async fn generate(&self, _prompts: &[String], _stop: Option<&[&str]>) -> Result<LLMResult> {
        let text = format!("ZhipuAI GLM stub response for model: {}", self.model);
        Ok(LLMResult {
