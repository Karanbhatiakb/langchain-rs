    ...
    async fn load_memory_variables(&self, inputs: &HashMap<String, Value>) -> Result<HashMap<String, Value>> {
        ...
        if input.is_empty() {
            if self.return_messages {
                let msgs: Vec<Value> = self.chat_history.read().iter()
                    .map(|m| serde_json::to_value(m).unwrap_or_default())
                    .collect();
                result.insert(self.memory_key.clone(), Value::Array(msgs));
            } else {
                result.insert(self.memory_key.clone(), Value::String(self.conversation_buffer()));
            }
            return Ok(result);
        }

        let query_embedding = self.embeddings.embed_query(input).await?;
        let similar_docs = self.vectorstore
            .similarity_search_by_vector(query_embedding, self.k)
            .await?;

        let memory_text = similar_docs.iter()
