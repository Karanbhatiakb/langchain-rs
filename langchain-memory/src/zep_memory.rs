//! Zep long-term memory — uses the Zep API for persistent conversation memory.
...
/// Long-term memory backed by the Zep API.
...
impl ZepMemory {
    ...
    pub async fn get_memory(&self) -> Result<Option<ZepMemoryResponse>> {
        match self
            .api_get(&format!("/api/v1/sessions/{}/memory", self.session_id))
            .await
        {
            Ok(body) => {
                let memory: ZepMemoryResponse = serde_json::from_str(&body).map_err(|e| {
                    ChainError::ParserError(format!("Failed to parse Zep memory: {}", e))
                })?;
                Ok(Some(memory))
            }
            Err(e) => {
    ...
    pub async fn add_memory(&self, messages: Vec<ZepMessage>) -> Result<()> {
        ...
        self.api_post(
            &format!("/api/v1/sessions/{}/memory", self.session_id),
            &body,
        )
        .await?;
        Ok(())
...
impl BaseMemory for ZepMemory {
    ...
    async fn clear(&self) -> Result<()> {
        self.api_delete(&format!("/api/v1/sessions/{}/memory", self.session_id))
            .await
    }
}
