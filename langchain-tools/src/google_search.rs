//! Google Search tool.

use async_trait::async_trait;
use serde_json::json;

use super::traits::{BaseTool, ToolResult};
use langchain_core::errors::ChainError;

pub struct GoogleSearchTool {
    api_key: String,
    base_url: String,
    client: reqwest::Client,
}

impl Default for GoogleSearchTool {
    fn default() -> Self {
        Self::new()
    }
}

impl GoogleSearchTool {
    pub fn new() -> Self {
        let api_key = std::env::var("SERPER_API_KEY").unwrap_or_default();
        Self {
            api_key,
            base_url: "https://google.serper.dev/search".into(),
            client: reqwest::Client::new(),
        }
    }

    pub fn with_api_key(mut self, key: &str) -> Self {
        self.api_key = key.to_string();
        self
    }

    pub fn with_base_url(mut self, url: &str) -> Self {
        self.base_url = url.to_string();
        self
    }
}

#[async_trait]
impl BaseTool for GoogleSearchTool {
    fn name(&self) -> &str {
        "google_search"
    }

    fn description(&self) -> &str {
        "Search Google using Serper API. Input should be a search query string."
    }

    async fn invoke(&self, input: &str) -> ToolResult {
        let query = input.trim();
        if query.is_empty() {
            return Err(ChainError::ToolError("Empty search query".into()));
        }
        if self.api_key.is_empty() {
            return Err(ChainError::ToolError("SERPER_API_KEY not set".into()));
        }
        let body = json!({ "q": query });
        let resp = self
            .client
            .post(&self.base_url)
            .header("X-API-KEY", &self.api_key)
            .header("Content-Type", "application/json")
            .json(&body)
            .send()
            .await
            .map_err(|e| ChainError::ToolError(format!("Serper API error: {}", e)))?;

        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().await.unwrap_or_default();
            return Err(ChainError::ToolError(format!("Serper API returned {}: {}", status, text)));
        }

        let result: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| ChainError::ToolError(format!("Parse error: {}", e)))?;

        let mut output = Vec::new();
        if let Some(organic) = result["organic"].as_array() {
            for (i, item) in organic.iter().take(5).enumerate() {
                let title = item["title"].as_str().unwrap_or("");
                let link = item["link"].as_str().unwrap_or("");
                let snippet = item["snippet"].as_str().unwrap_or("");
                output.push(format!("{}. {} - {}\n {}", i + 1, title, link, snippet));
            }
        }
        if let Some(answer_box) = result.get("answerBox") {
            if let Some(text) = answer_box["snippet"].as_str() {
                output.insert(0, format!("Answer: {}", text));
            }
        }
        if output.is_empty() {
            Ok("No results found".into())
        } else {
            Ok(output.join("\n"))
        }
    }
}
