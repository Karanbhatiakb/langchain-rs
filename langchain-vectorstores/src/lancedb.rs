...
impl VectorStore for LanceDBVectorStore {
    async fn add_texts(
        ...
        let ids: Vec<String> = (0..texts.len()).map(|i| format!("lance_{}", i)).collect();
        ...
        let resp = client
            ...
            .send()
            .await
            .map_err(|e| ChainError::VectorStoreError(format!("LanceDB insert error: {}", e)))?;

        if !resp.status().is_success() {
            return Err(ChainError::VectorStoreError(format!(
                "LanceDB insert failed: {}",
                resp.status()
            )));
        }
    ...
    async fn similarity_search_by_vector(
        ...
        let resp = client
            ...
            .send()
            .await
            .map_err(|e| ChainError::VectorStoreError(format!("LanceDB search error: {}", e)))?;
        ...
        let result: Value = resp
            .json()
            .await
            .map_err(|e| ChainError::VectorStoreError(format!("LanceDB parse error: {}", e)))?;
    ...
    async fn delete(&self, ids: Vec<String>) -> Result<()> {
        ...
        let resp = client
            ...
            .send()
            .await
            .map_err(|e| ChainError::VectorStoreError(format!("LanceDB delete error: {}", e)))?;

        if !resp.status().is_success() {
            return Err(ChainError::VectorStoreError(format!(
                "LanceDB delete failed: {}",
                resp.status()
            )));
        }
        Ok(())
